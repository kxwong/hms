...
public class retrieveAllCase extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (request.getParameter("ob") != null && request.getParameter("seq") != null) {
                ...
                issueList = em.createQuery("SELECT i FROM Issue i where i.status = :status " + "ORDER BY " + orderBy + " " + seq).setParameter("status", "Closed").getResultList();
                session.setAttribute("seq", seq.toLowerCase());
            } else {
                issueList = issueMgr.findAllByStatus("Closed");
            }

            session.setAttribute("issueList", issueList);
            response.sendRedirect("/hosteller/reporting/caseSearching");
