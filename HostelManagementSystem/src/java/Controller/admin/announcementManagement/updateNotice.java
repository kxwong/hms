    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (title.equals("")) {
                ...
                Announcement tempoNotice = (Announcement) session.getAttribute("notice");
                Announcement notice = announcementManager.findAnnouncementByID(tempoNotice.getAnnounceID());
                if (notice != null) {
                    ...
                    utx.begin();
                    announcementManager.updateAnnouncement(notice);
                    utx.commit();
                    session.setAttribute("success", "Notice is updated successfully");
                    response.sendRedirect("/retrieveNoticeDetails?p=" + crypto.ANEncode(notice.getAnnounceID()));
                } else {
                    throw new Exception("Selected notice is no existed");
                }
            }
        } catch (Exception ex) {
            session.setAttribute("error", ex.getLocalizedMessage());
            Announcement tempoNotice = (Announcement) session.getAttribute("notice");
            response.sendRedirect("/retrieveNoticeDetails?p=" + crypto.ANEncode(tempoNotice.getAnnounceID()));
        }
