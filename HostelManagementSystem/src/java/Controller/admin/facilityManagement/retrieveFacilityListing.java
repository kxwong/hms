...
public class retrieveFacilityListing extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        for (int i = 0; i < locationList.size(); i++) {
            List<String> tempoBuildingList = facilityManager.findAllBuilding(locationList.get(i));
            allBuildingList.add(tempoBuildingList);
        }
        session.setAttribute("facilityList", facilityManager.findAll());
        session.setAttribute("locationList", locationList);
        session.setAttribute("buildingList", allBuildingList);
        response.sendRedirect("/admin/facility/facilityListing");
    }
