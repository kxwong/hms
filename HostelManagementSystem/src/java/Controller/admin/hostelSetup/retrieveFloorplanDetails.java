...
public class retrieveFloorplanDetails extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (floorplanManager.findFloorplanByID(floorplanID) != null) {
                session.setAttribute("floorplan", floorplanManager.findFloorplanByID(floorplanID));
                session.setAttribute("mgr",mgr);
                response.sendRedirect("/admin/hostel/viewFloorplan");
            } else {
                throw new Exception("Selected floorplan is not existed");
            }
        } catch (Exception ex) {
