...
<html>
    <head>
        ...
        <link rel="stylesheet" href="../../managementSide/source/css/issueBill.css">
    ...
    <body>
        ...
        <div class="content">
            <div class="cHeader">
                <div class="chTitle">Billing's Overview &#10148; Issue Bill</div>
            </div>
            <div class="cBody">
                <form action="/storeBill" method="post">
                    <div class="cbContent">
                        <div class="cbInstuction">
                            <div class="cbLabel">Issue Bill Month: <%=dateString.ToStringMonth(issueDate.getMonth()) + " " + String.valueOf(issueDate.getYear() + 1900)%> </div>
                        </div><br>
                        <div class="cbInstuction">
                            <div class="cbLabel">Select the target hosteller(s) to issue bill.</div>
                            <button type="button" onclick="selectAll()" class="cbFunc">Select All</button>
                        </div>
                        <div class="cbSelectionArea">
                            <%
                                for (int i = 0; i < hostellerList.size(); i++) {
                                    String call = "ID : " + hostellerList.get(i).getHostellerID() + " Name : " + hostellerList.get(i).getFirstName().charAt(0) + (hostellerList.get(i).getMiddleName().equals("")?"": "." + hostellerList.get(i).getMiddleName().charAt(0)) + "." + hostellerList.get(i).getLastName();
                            %>
                            <div id="<%=i%>" class="cbSelection" onclick="loadHosteller('<%=hostellerList.get(i).getHostellerID()%>', '<%=i%>')">
                                <i id="<%=hostellerList.get(i).getHostellerID()%><%=i%>" class="cbBox cbChecked">&#10004;</i> <%=call%>
                            </div>
                            <input id="<%=hostellerList.get(i).getHostellerID()%>" style="display:none" type="checkbox" name="<%=hostellerList.get(i).getHostellerID()%>" checked="true">
                            <%
                                }
                            %>
                        </div>
                        <a href="/retrieveBillListing?t=0"><button type="button" class="cbAdd">&#9166; Back</button></a> 
                        <button class="cbAdd cBAddright">Issue</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script type="text/javascript">
        function loadHosteller(id, i) {
            var x = document.getElementById(id).checked;
            var v = id + i;
            if (x === true) {
                document.getElementById(id).checked = false;
                document.getElementById(v).setAttribute('class', "cbBox");
                document.getElementById(i).setAttribute('class', "cbSelection cbUnselected");
            } else {
                document.getElementById(id).checked = true;
                document.getElementById(v).setAttribute('class', "cbBox cbChecked");
                document.getElementById(i).setAttribute('class', "cbSelection");
            }
        }
        function selectAll() {
        <%
            String array = "";
            for (int i = 0; i < hostellerList.size(); i++) {
                array += '"' + hostellerList.get(i).getHostellerID() + '"' + ",";
            }
            array = array.concat(",");
            array = array.replace(",,", "");
        %>
            ...
            for (i = 0; i < array.length; i++) {
                ...
                document.getElementById(array[i] + i).setAttribute('class', "cbBox cbChecked");
                document.getElementById(i).setAttribute('class', "cbSelection");
            }
        }
