                    ...
                    <form action="/updateRoom" method="post">
                        <table>
                            <tr>
                                ...
                                <td> : </td>
                                <td><input  class="cbInput" name="desc" type="text" min="1" value="<%=room.getDescription()%>"></td>
                                <td rowspan="5">
                                    <img class="cbPreview" id="preview" src="<%="data:image/jpeg;base64," + new String(Base64.getEncoder().encode(room.getImage()))%>" />
                                </td>
                            </tr>
                            <tr>
                                <td><div class="cbLabel">Rental</div></td>
                                <td> : </td>
                                <td><input class="cbInput" name="rental" type="number" min="200" step="10" max="1000" value="<%=room.getRentalFee()%>" /></td>
                            </tr>
                            <tr>
                                <td><div class="cbLabel">Capacity</div></td>
                                <td> : </td>
                                <td><input  class="cbInput" name="capacity" type="number" min="1" value="<%=room.getCapacity()%>"></td>
                            </tr>
                            <tr>
                                <td><div class="cbLabel">Image</div></td>
                                <td> : </td>
                                <td>
                                    <input type="file" id="files" name="roomImg" onchange="fileValidation(this, event)" accept="image/*" >
                                    <input type="text" name="imageByte" id="byte_content" hidden value="<%="data:image/jpeg;base64," + new String(Base64.getEncoder().encode(room.getImage()))%>">
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td> : </td>
                                <td>
                                    <select class="cbInput" name="status">
                                        <%
                                            String available = "";
                                            String notAvailable = "";
                                            if(room.getStatus().toLowerCase().equals("available")){
                                                available = "selected='selected'";
                                            }else{
                                                notAvailable = "selected='selected'";
                                            }
                                        %>
                                        <option value="Available" <%=available%> >Available</option>
                                        <option value="Not Available" <%=notAvailable%> >Not Available</option>
                                    </select>
                                </td>
                            </tr>
                        </table>
                        <a href="/retrieveRoomDetail?p=<%=crypto.REncode(room.getRoomNo())%>"><button type="button" class="cbAdd">&#9166; Back</button></a> 
