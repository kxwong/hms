...
<%@page import="Model.RegistrationreqManager"%>
...
<%
    try {
        ...
        List<Entrecord> tenantEntryRecordList = (List) session.getAttribute("tenantEntryRecordList");
    ...
    <body>
        <div class="header">
            <div class="hCover">
                ...
                <a href='/retrieveTenantEntryListing?t=0'><div class="hBG"><div class="hOption hChoose"><p>Tenant Entrance</p></div></div></a>
                <div class="hSubcover"><div class="hSpaceRight"></div></div>
                <a href='/retrieveVisitorEntryListing?t=0'><div class="hBG"><div class="hOption"><p>Visitor Entrance</p></div></div></a>
            </div>
        ...
        <div class="content">
            <div class="cHeader">
                <div class="chTitle">Tenant Entrance Record's Overview</div>
            </div>
            <div class="cBody">
                <div class="cbContent cbStatus">
                    <div class="cbsOption  <%=allClass%>"><a href="/retrieveTenantEntryListing?t=1&s=<%=crypto.CEncode("All")%>"><center>All</center></a></div>
                    <div class="cbsOption  <%=entryClass%>"><a href="/retrieveTenantEntryListing?t=1&s=<%=crypto.CEncode("Entry")%>"><center>Entry</center></a></div>
                    <div class="cbsOption  <%=leaveClass%>"><a href="/retrieveTenantEntryListing?t=1&s=<%=crypto.CEncode("Leave")%>"><center>Leave</center></a></div>
                </div>
                <div class="cbContent cbSearchCriteria">
                    <form action="/retrieveTenantEntryListing?t=2" method="post">
                ...
                <div class="cbContent cbList">
                    <table>
                        <thead>
                            <tr>
                                <th><a href="/retrieveTenantEntryListing?t=3&o=<%=crypto.CEncode("e.entRecordID")%>">Entrance ID</a></th>
                                <th><a href="/retrieveTenantEntryListing?t=3&o=<%=crypto.CEncode("e.entCardNo.hosteller.firstName, e.entCardNo.hosteller.middleName, e.entCardNo.hosteller.lastName")%>">Access By</a></th>
                                <th><a href="/retrieveTenantEntryListing?t=3&o=<%=crypto.CEncode("e.accessTime")%>">Time</a></th>
                                <th><a href="/retrieveTenantEntryListing?t=3&o=<%=crypto.CEncode("e.gate")%>">Gate</a></th>
                                <th><a href="/retrieveTenantEntryListing?t=3&o=<%=crypto.CEncode("e.hostel.location")%>">Location</a></th>
                                <th><a href="/retrieveTenantEntryListing?t=3&o=<%=crypto.CEncode("e.hostel.building")%>">Building</a></th>
                            </tr>
                        </thead>
                        <tbody>
                            <%
                                HostellerManager hostellermanager = new HostellerManager((EntityManager) session.getAttribute("mgr"));
                                for (int i = 0; i < tenantEntryRecordList.size(); i++) {
                                    try {
                                        Entrecord entryRecord = tenantEntryRecordList.get(i);
                                        Hosteller hosteller = hostellermanager.findByEntCardNo(entryRecord.getEntCardNo().getEntCardNo());
                                        if (hosteller.getRegReqNo().getUpdateDate().after(entryRecord.getAccessTime())) {
                                            throw new Exception();
                                        }
                                        String hostellerID = crypto.HTEncode(hosteller.getHostellerID());
                            %>
                            <tr>
                                <td><%=entryRecord.getEntRecordID()%></td>
                                <td><a href="/retrieveTenantDetails?p=<%=hostellerID%>" class="cbrlRequest"><%=hosteller.getFirstName()%> <%=hosteller.getMiddleName()%> <%=hosteller.getLastName()%></a></td>
                            ...
                            %>
                            <% if (tenantEntryRecordList.isEmpty()) {%>
                            <tr>
